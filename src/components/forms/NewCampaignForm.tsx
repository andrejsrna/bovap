    ...
    <section className="rounded-xl border border-gray-200 bg-gray-50/70 p-4 sm:p-5">
      ...
      <div className="space-y-4">
        {cards.map((card, index) => <div key={index} className="rounded-lg border border-gray-200 bg-white p-4">
          ...
          <div className="space-y-3"><Input value={card.title} onChange={(e) => update(index, "title", e.target.value)} placeholder="Názov karty" /><Textarea value={card.description} onChange={(e) => update(index, "description", e.target.value)} rows={3} placeholder="Stručný popis" /><Input value={card.url} onChange={(e) => update(index, "url", e.target.value)} type="url" placeholder="https://…" /></div>
        </div>)}
      </div>
